...
shell-tools-debug() {
    ...
    echo "模块加载状态:"
    if [[ -n "${SHELL_TOOLS_MODULES_LOADED}" ]] && [[ ${#SHELL_TOOLS_MODULES_LOADED[@]} -gt 0 ]]; then
        for module in "${(@k)SHELL_TOOLS_MODULES_LOADED}"; do
            echo "  ✓ $module"
        done
    else
        echo "  无已加载模块"
    ...
    if [[ -n "${SHELL_TOOLS_MODULES_FAILED}" ]] && [[ ${#SHELL_TOOLS_MODULES_FAILED[@]} -gt 0 ]]; then
        echo
        echo "模块加载失败:"
        for module in "${(@k)SHELL_TOOLS_MODULES_FAILED}"; do
            echo "  ✗ $module"
        done
    fi
