...
apply_zshrc_changes() {
    ...
    script_content="# SSH Agent 自动管理（防止重复启动）
    SSH_AGENT_FILE=\"\$HOME/.ssh-agent-ohmyzsh\"

    # 启动新的 ssh-agent 并添加密钥
    _start_ssh_agent() {
        ssh-agent -s -t 12h > \"\$SSH_AGENT_FILE\"
        eval \"\$(cat \"\$SSH_AGENT_FILE\")\"
        # 只添加存在的私钥文件（跳过 .pub 文件和目录）
        for key in ~/.ssh/id_*. ~/.ssh/*.pub; do
            [[ -f \"\$key\" && \"\$key\" != *.pub ]] && ssh-add \"\$key\" 2>/dev/null
        done
    }

    # 1. 检查当前 shell 是否已有可用的 SSH agent
    if [ -S \"\${SSH_AUTH_SOCK}\" ]; then
        # 已有可用的 agent，无需操作
        :
    # 2. 尝试加载已保存的 agent 配置
    elif [ -f \"\$SSH_AGENT_FILE\" ]; then
        eval \"\$(cat \"\$SSH_AGENT_FILE\")\" 2>/dev/null
        # 3. 验证加载的 agent 是否真的存活（检查 socket 和 PID）
        if [ ! -S \"\${SSH_AUTH_SOCK}\" ] || ! kill -0 \"\${SSH_AGENT_PID:-}\" 2>/dev/null; then
            # agent 已死或无效，启动新的
            _start_ssh_agent
        fi
    else
        # 4. 没有保存的配置，直接启动新的 agent
        _start_ssh_agent
    fi"
